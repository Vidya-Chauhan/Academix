...
const Login = ({ onLogin }) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-cover bg-center"
         style={{ backgroundImage: `url(${LoginBg})` }}>
      <div className="relative w-[400px] h-[500px] bg-[#F5EFE8]/80 rounded-2xl shadow-2xl backdrop-blur-md p-6 flex flex-col items-center border border-white border-opacity-20">
        
        <div className="absolute top-0 left-0 w-full bg-[#C0D9E6] p-4 rounded-t-2xl text-center shadow-md">
          <img src={NITLogo} alt="NIT Logo" className="w-36 mx-auto rounded" />
        </div>

        <h2 className="mt-24 text-xl font-semibold text-[#2F4156] text-center font-serif">
        ...
        <form onSubmit={handleLogin} className="w-full flex flex-col items-center mt-4">
          <div className="w-4/5 mb-4">
            <input
              type="email"
              className="w-full p-3 rounded-lg bg-white text-[#2F4156] border border-[#C0D9E6] focus:outline-none focus:ring-2 focus:ring-teal-400 transition-all duration-200 placeholder:text-gray-500"
              placeholder="Email (e.g., bt21cse001@nituk.ac.in)"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              required
            />
          ...
          <button
            type="submit"
            className="w-4/5 py-3 rounded-md bg-[#2F4156] text-white font-semibold hover:bg-[#576C8D] transition duration-300 ease-in-out transform hover:scale-105 shadow-md"
          >
            Login
          </button>
        </form>

        <p className="text-sm text-[#2F4156] mt-4">
          Don't have an account?{" "}
          <button
